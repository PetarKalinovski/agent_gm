...
| Walk animation | 6-frame filmstrip cycles, ONE Gemini call per direction, left mirrored from right (models draw walkers facing right no matter what you ask); artifact-immune slicing; frame validation + phase-partner repair; normalized to idle framing with feet-centroid anchoring | `src/services/image_generator.py` (`generate_walk_cycle`) |
