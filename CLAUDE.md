...
- **ImageGenerator** (`image_generator.py`) - Generates sprites, portraits, and location backgrounds via LLM image APIs. Walk animations are 6-frame cycles generated as a single filmstrip per direction (front/back/right; left is mirrored from right — image models have a left-to-right walking bias and flip requested left-facing strips, so we generate the direction they're biased toward; the left idle is likewise a mirror of the right idle so it matches the mirrored walk frames) — one call per direction keeps frames style-consistent, then the strip is sliced (artifact-immune: full-span rows/columns like grid lines and letterbox cards are cleared before the content bbox), background-removed, validated (ghost/sliver cells are replaced by their phase partner; a mostly-broken strip regenerates once), and normalized to the idle sprite's character-height ratio and baseline with feet-centroid horizontal anchoring so frames render at identical size with feet planted.
